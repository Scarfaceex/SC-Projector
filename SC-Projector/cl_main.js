...
on('leo-projector:client:project', async () => {
  ...
  if (keyboard.link) emitNet('leo-projector:server:project-start', keyboard.link)
})
on('leo-projector:client:project-clear', () => {
  emitNet('leo-projector:server:project-stop')
})

RegisterNetEvent('leo-projector:client:project-start')
RegisterNetEvent('leo-projector:client:project-stop')
on('leo-projector:client:project-start', async (link) => {
  if (currentUrl) CleanupDUI()
  currentUrl = link

  await taskPlayUrl(link)
})
on('leo-projector:client:project-stop', () => {
  currentUrl = null
  CleanupDUI()
})
...
const SetupQbTarget = () => {
  exports['qb-target'].AddTargetModel(GetHashKey('p_planning_board_02'), {
    options: [
      {
        event: 'leo-projector:client:project',
        icon: 'fas fa-display',
        label: 'Use Projector',
      },
      {
